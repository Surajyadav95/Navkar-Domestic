...
Partial Class Summary_BCYMovement
    ...
    Protected Sub Filldropdown()
        Try
            ds = db.sub_GetDataSets("usp_empty_summary_search")
            If (ds.Tables(0).Rows.Count > 0) Then
                ddlLine.DataSource = ds.Tables(0)
    ...
    Protected Sub btnsearch_Click(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += " usp_domestic_Loaded_Summary_In_Search '" & Convert.ToDateTime(txtfromDate.Text).ToString("dd MMM yyyy 00:00:00 ") & "','" & Convert.ToDateTime(txttoDate.Text).ToString("dd MMM yyyy 23:59:00 ") & "',"
            strSql += "'" & Trim(ddlCategory.SelectedValue & "") & "','" & Trim(ddlLine.SelectedValue & "") & "','" & Trim(ddlCustomer.SelectedValue & "") & "'"
        ...
        Try
            ...
            If (dt.Rows.Count > 0) Then
                Using wb As New XLWorkbook()
                    wb.Worksheets.Add(dt, "Loaded In Summary" & Convert.ToDateTime(Now).ToString("dd-MM-yyyy") & "")
                    ...
                    With wb.Worksheets(0)
                        ...
                        .Cell(11, 1).Value = "Loaded In Summary"
                    ...
                    Response.AddHeader("content-disposition", "attachment;filename=LoadedInSummary" & Convert.ToDateTime(Now).ToString("ddMMyyyyHHmm") & ".xlsx")
    ...
    Protected Sub lnkCancel_Click(sender As Object, e As EventArgs)
        Try
            ...
            Dim GateInNo As String = lnkcancel.CommandArgument
            Dim ContainerNo As String = grdcontainer.DataKeys(row.RowIndex).Value.ToString()
            strSql = ""
            strSql += "select * from Domestic_Stock where entryID='" & GateInNo & "' and containerNo='" & ContainerNo & "' and outdate is null and DestuffDate is null"
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
                strSql = ""
                strSql += "USP_DELETE_FROM_DOMESTIC_IN '" & GateInNo & "','" & ContainerNo & "'"
                db.sub_ExecuteNonQuery(strSql)
                db.AmmendmentLog(strSql, Session("UserId_DomCFS"), "Domestic Loaded In")
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "Key", "alert('Container cancelled successfuly');", True)
                btnsearch_Click(sender, e)
            Else
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "Key", "alert('Container cannot be cancelled because either it is out or destuffed');", True)
                Exit Sub
            End If
