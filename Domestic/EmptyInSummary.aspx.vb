...
Partial Class Summary_BCYMovement
    ...
    Protected Sub btnsearch_Click(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += " usp_empty_search_in '" & Convert.ToDateTime(txtfromDate.Text).ToString("dd MMM yyyy 00:00:00 ") & "','" & Convert.ToDateTime(txttoDate.Text).ToString("dd MMM yyyy 23:59:00 ") & "',"
            strSql += "'" & Trim(ddlCategory.SelectedValue & "") & "','" & Trim(ddlLine.SelectedValue & "") & "','" & Trim(txtcontainer.Text) & "'"
        ...
        Try
            ...
            If (dt.Rows.Count > 0) Then
                Using wb As New XLWorkbook()
                    wb.Worksheets.Add(dt, "Empty In Summary" & Convert.ToDateTime(Now).ToString("dd-MM-yyyy") & "")
                    ...
                    With wb.Worksheets(0)
                        ...
                        .Cell(11, 1).Value = "Empty In Summary"
                    ...
                    Response.AddHeader("content-disposition", "attachment;filename=EmptyInSummary" & Convert.ToDateTime(Now).ToString("ddMMyyyyHHmm") & ".xlsx")
    ...
    Protected Sub lnkCancel_Click(sender As Object, e As EventArgs)
        Try
            ...
            strSql = ""
            strSql += "select * from Domestic_EmptyStock where entryID='" & GateInNo & "' and containerNo='" & ContainerNo & "' and outdate is null and stuffingdate is null"
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
                strSql = ""
                strSql += "USP_DELETE_FROM_DOMESTIC_EMPTY_IN '" & GateInNo & "','" & ContainerNo & "'"
                db.sub_ExecuteNonQuery(strSql)
                db.AmmendmentLog(strSql, Session("UserId_DomCFS"), "Domestic Empty In")
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "Key", "alert('Container cancelled successfuly');", True)
                btnsearch_Click(sender, e)
            Else
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "Key", "alert('Container cannot be cancelled because either it is out or stuffed');", True)
                Exit Sub
            End If
        Catch ex As Exception
            lblError.Text = "Error in procedure: " & Reflection.MethodBase.GetCurrentMethod.Name & ". " & ex.Message.ToString
