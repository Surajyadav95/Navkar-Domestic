...
Partial Class Summary_BCYMovement
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        Try
            strSql += " USP_DOMESTIC_INVOICE_DETAILS_Proforma '" & Convert.ToDateTime(txtfromDate.Text).ToString("yyyy-MM-dd HH:mm") & "','" & Convert.ToDateTime(txttoDate.Text).ToString("yyyy-MM-dd HH:mm") & "','" & Trim(ddlCustomer.SelectedValue) & "','" & Trim(txtInvoiceSearch.Text) & "'"
    ...
    Protected Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        Try
            ...
            End If
            strSql = ""
            strSql += " USP_DOMESTIC_INVOICE_DETAILS_Proforma '" & Convert.ToDateTime(txtfromDate.Text).ToString("yyyy-MM-dd HH:mm") & "','" & Convert.ToDateTime(txttoDate.Text).ToString("yyyy-MM-dd HH:mm") & "','" & Trim(ddlCustomer.SelectedValue) & "'"
            dt = db.sub_GetDatatable(strSql)
            grdcontainer.DataSource = dt
            grdcontainer.DataBind()
            strSql = ""
    ...
    Protected Sub lnkSumbit_Click(sender As Object, e As EventArgs)
        Try
            Dim lnkcancel As LinkButton = DirectCast(sender, LinkButton)
            Dim row As GridViewRow = DirectCast(lnkcancel.Parent.Parent, GridViewRow)
            Dim AssessNo As String = lnkcancel.CommandArgument
            ...
            strSql = ""
            strSql += "USP_INSERT_INTO_Domestic_Final'" & AssessNo & "','" & WorkYear & "'"
            db.sub_ExecuteNonQuery(strSql)
            btnSave_Click(sender, e)
            lblsession.Text = "Assessment Sumbit Successfully"
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModalforupdate2", "$('#myModalforupdate2').modal();", True)
            'UpdatePanel6.Update()
            'lientScript.RegisterStartupScript(Page.GetType(), "OpenList", "<script>OpenCancelInvoice(); </script>")
        Catch ex As Exception
